...
CONFIG_HELPER=scripts/cfg_herlper.py

SRC_DIR=../dns-ctrl-uc/sigcomm/sdnres-sigcomm/
SUBDIR=$(date "+%d-%m-%Y_%Z_%H:%M:%S.%N")/
LOG_DIR=log/$(SUBDIR)/
...
start:
	...
	mkdir -p $(LOG_DIR)
	./$(CONFIG_HELPER) --topo $(TOPOLOGY) --log $(LOG_LEVEL) --src-dir $(SRC_DIR) --log-dir $(LOG_DIR) --pid $(PIDFILE)
