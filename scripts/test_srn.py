...
def test_flapping_link():
    ...
    try:
        ...
        time.sleep(10)
        cmd = [sr_testdns, "sr", "1", server.name + ".test.sr", dns_proxy_ip6]
        print(" ".join(cmd))
        out = client.cmd(cmd)
        print(out)

        # Insert one encapsulation rule so that pings to server uses the binding segment

        server_ip6 = None
        for itf in realIntfList(server):
            for ip6 in itf.ip6s(exclude_lls=True):
                server_ip6 = ip6.ip.compressed
                lg.debug("server address found was %s", server_ip6)
        if server_ip6 is None:
        ...
        cmd = ["ip", "-6", "route", "add", server_ip6, "encap", "seg6", "mode", "inline", "segs", bsid,
               "dev", "client-eth0"]
        print(" ".join(cmd))
        out = client.cmd(cmd)
        print(out)
